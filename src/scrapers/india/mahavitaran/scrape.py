    ...
    def scrape(self):
        ...
        try:
            ...
            WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.ID, "shd_select"))
            )

            # Select the current month
            Select(driver.find_element(By.ID, "shd_select")).select_by_visible_text(
                target_month
            )

            # Set table to show all entries (avoid pagination)
            WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.NAME, "wo_shd_table_length"))
            )
            select_length = Select(driver.find_element(By.NAME, "wo_shd_table_length"))
            # Try to select "All" or the highest number available
            options = [
                option.get_attribute("value") for option in select_length.options
            ]
            ...
            WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.ID, "wo_shd_table"))
            )
