...
def update_current_outages(pre_existing_outages, new_data):
    ...
    for outage in new_data:
        ...
        # "affected_client", add to "updated_current_outages" and move on to next outage
        updated_outage = dict(existing)
        updated_outage["attributes"] = dict(existing["attributes"])
        updated_outage["attributes"]["affected_client"] = max(
            0,
            existing["attributes"]["affected_client"],
            outage["attributes"]["affected_client"],
        )
    ...
    for id in all_ids:
        ...
        # if outage is ongoing
            ...
            num_people_affected = max(
                0,
                previous_outage_data["attributes"]["affected_client"],
                new_outage_data["attributes"]["affected_client"],
            )
            outage = new_outage_data
            outage["attributes"]["affected_client"] = num_people_affected
            outage["outage_ended_today"] = False
            updated_raw_outage_data.append(outage)
...
if __name__ == "__main__":
    ...
    pre_existing_raw_outages = get_pre_existing_raw_outages(
        current_datetime, bucket_name="spain", provider_name="edistribucion"
    )
    ...
    upload_raw(
        ...
        current_datetime,
        bucket_name="spain",
        provider_name="edistribucion",
    )

    # get pre-existing outages from minio
    get_pre_existing_current_outages(bucket_name="spain", provider_name="edistribucion")
    ...
    upload_current_outages(
        updated_current_outages, bucket_name="spain", provider_name="edistribucion"
    )
