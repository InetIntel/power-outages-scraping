# Spain Power Outage Data (Iberdrola / i-DE)
...
Municipio - (English: municipality; corrected 2026-07-22, was "Municipo") - string containing areas affected by the outage. if there are multiple areas then 
...
Source page: `https://www.i-de.es/outages-scheduled-power-cuts/scheduled-power-cuts`
...
their pages (`get_region_links()`). From each of these region specific pages, the scraper collects the download link for the 
pdf file containing all the planned outages in that region for the week (`get_region_pdfs()`).

**Important gap found 2026-07-22:** although `get_region_links()` collects every region, `__main__` only passes the first 5 to `get_region_pdfs()` — `spider.get_region_pdfs(links[:5])`. In production this means only 5 regions' PDFs are ever fetched, not "all the individual regions covered by the provider" as the code's own reachable logic would otherwise allow. This looks like a debug/test leftover (`links[:5]`) that was never removed, and it silently limits data coverage. Verify against a live run and fix or document intentionally before relying on full-country coverage.
...
storage. While uploading, it skips files whose name starts with `"SGS "` (corrected 2026-07-22, was described as "containing SGS" — the actual check is `file.startswith("SGS ")`), these are occasional stray file downloads, I'm 
