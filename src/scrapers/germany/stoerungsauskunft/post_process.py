...
BUCKET_NAME = "germany"
PROVIDER_NAME = "stoerungsauskunft"
COUNTRY_CODE = "DE"
...
def deduplicate(data):
    """
    Deduplicate outage records based on incidentreference.
    Keeps the last occurrence of each unique incidentreference.
    """
    seen = {}
    for record in data:
        key = record.get("incidentreference")
        if key is not None:
            seen[key] = record
        else:
            # Keep records without an incidentreference as-is
