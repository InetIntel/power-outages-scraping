...
def post_process(date_str=None):
    ...
    raw_filename = f"power_outages.GB.national_grid.raw.{date_str}.json"
    raw_path = f"national_grid/raw/{year}/{month}/{raw_filename}"
    ...
    # Save processed JSON
    processed_filename = f"power_outages.GB.national_grid.processed.{date_str}.json"
    local_processed = f"/tmp/{processed_filename}"
    with open(local_processed, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

    processed_path = f"national_grid/processed/{year}/{month}/{processed_filename}"
