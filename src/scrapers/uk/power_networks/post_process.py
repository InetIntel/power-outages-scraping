...
def post_process(date_str=None):
    ...
    raw_filename = f"power_outages.GB.power_networks.raw.{date_str}.json"
    raw_path = f"power_networks/raw/{year}/{month}/{raw_filename}"
    ...
    # Save processed JSON
    processed_filename = f"power_outages.GB.power_networks.processed.{date_str}.json"
    local_processed = f"/tmp/{processed_filename}"
    with open(local_processed, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

    processed_path = f"power_networks/processed/{year}/{month}/{processed_filename}"
