...
class OkinawaProcessor:
        ...
        for town in root.findall(".//town"):
            try:
                ...
                if not (
                    power_cut_date
                    and power_cut_time
                    and send_finish_date
                    and send_finish_time
                ):
                    continue  # skip incomplete entries

                # Combine date + time into ISO format
                try:
                    occurrence_dt = datetime.strptime(
                        power_cut_date + " " + power_cut_time, "%Y.%m.%d %H:%M"
                    )
                ...
                try:
                    recovery_dt = datetime.strptime(
                        send_finish_date + " " + send_finish_time, "%Y.%m.%d %H:%M"
                    )
    ...
    def run(self):
        ...
        prefix = f"japan/okinawa/raw/{year}/{month}/"
        listing = uploader.client.list_objects_v2(Bucket="japan", Prefix=prefix)
        for obj in listing.get("Contents", []):
            key = obj["Key"]
            local_path = self.base_path / Path(key).name
        ...
        s3_processed = f"japan/okinawa/processed/{year}/{month}/{self.output_file.name}"
        uploader.upload_file(str(self.output_file), s3_processed)
