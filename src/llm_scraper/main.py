...
def fetch_countries():
    ...
    try:
        ...
        data = response.json()
        country_to_code = {country["name"]: country["code"] for country in data["data"]}
...
def fetch_regions_for_country(country_name, country_code):
    ...
    """
    params = {"entityType": "region", "relatedTo": f"country/{country_code}"}
    headers = {"accept": "*/*"}
...
def create_notion_database(
    notion_client, main_page_id, country_name, existing_databases
):
    """
...
Do NOT include markdown, code blocks, or any explanation. Only return pure JSON.""",
        ...
        if not isinstance(data, list):
            print(
                f"  [WARN] {country_name}: Parser returned non-list, converting to list"
            )
        ...
        query = notion_client.databases.query(
            ...
            filter={
                "and": [
                    ...
                    {"property": "Region", "rich_text": {"equals": region}},
                ]
            },
        )
        ...
        try:
            ...
            notion_client.pages.create(
                ...
                properties={
                    ...
                    "Description": {
                        "rich_text": [
                            {"text": {"content": company.get("Description", "")}}
                        ]
                    },
                        ...
                        "rich_text": [
                            {
                                "text": {
                                    "content": company.get(
                                        "Power Outage Link", company.get("Link", "")
                                    )
                                }
                            }
                        ]
                    ...
                    "Challenges": {
                        "multi_select": [
                            {
                                "name": clean_multiselect_value(
                                    company.get("Challenges", "")
                                )
                            }
                        ]
                    },
                    ...
                    "Scraping Frequency": {
                        "multi_select": [
                            {
                                "name": clean_multiselect_value(
                                    company.get("Scraping Frequency", "Once")
                                )
                            }
                        ]
                    },
                    "Status": {
                        "multi_select": [
                            {
                                "name": clean_multiselect_value(
                                    company.get("Status", "Not Started")
                                )
                            }
                        ]
                    },
            ...
            print(
                f"    [ERROR] Error adding {company.get('Company / Data source', 'Unknown')}: {e}"
            )
...
def scrape_and_populate_all_countries(country_to_database, max_workers=2):
    ...
    def process_country(country_name, database_id):
        ...
        print(
            f"  [INFO] {country_name}: Received {len(companies_data)} companies from ChatGPT Data:{str(companies_data)[:100]}..."
        )
        ...
        # Add to Notion
        added_count = add_companies_to_notion(
            notion, database_id, country_name, companies_data
        )
...
Examples:
    ...
    parser.add_argument(
        "--scrape", action="store_true", help="Enable scraping for all countries"
    )
    ...
    if args.country:
        matching_countries = [
            c for c in country_to_code.keys() if c.lower() == args.country.lower()
        ]
    ...
    if not args.scrape_only:
        ...
        created_databases = create_all_notion_databases(
            country_to_region, max_workers=3
        )
    else:
    ...
    if should_scrape:
        ...
        if notion_token and main_page_id:
            ...
            print("\n" + "=" * 60)
            scrape_stats = scrape_and_populate_all_countries(
                country_to_database, max_workers=2
            )
            ...
            print(
                f"  Total companies found: {scrape_stats.get('total_companies_found', 0)}"
            )
            print(
                f"  Total companies added: {scrape_stats.get('total_companies_added', 0)}"
            )
