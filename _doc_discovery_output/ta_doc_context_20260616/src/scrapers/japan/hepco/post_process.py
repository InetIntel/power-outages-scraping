...
class HEPCOProcessor:
    def __init__(self):
        ...
        self.raw_filename = (
            f"power_outages.JP.{self.provider}.raw.{self.today_iso}.html"
        )
        self.processed_filename = (
            f"power_outages.JP.{self.provider}.processed.{self.today_iso}.json"
        )
        ...
        for r in raw_records:
            try:
                start_dt = datetime.strptime(
                    f"{r['start_date']} {r['start_time']}", "%Y/%m/%d %H:%M"
                )
                end_dt = datetime.strptime(
                    f"{r['end_date']} {r['end_time']}", "%Y/%m/%d %H:%M"
                )
    ...
    def run(self):
        ...
        s3_path = f"japan/hepco/raw/{self.year}/{self.month}/{self.raw_filename}"
        try:
            self.raw_path.parent.mkdir(parents=True, exist_ok=True)
            uploader.download_file(s3_path, str(self.raw_path))
        ...
        s3_processed = (
            f"japan/hepco/processed/{self.year}/{self.month}/{self.processed_filename}"
        )
        uploader.upload_file(str(self.output_path), s3_processed)
