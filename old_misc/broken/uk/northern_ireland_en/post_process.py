...
def post_process(date_str=None):
    ...
    raw_filename = f"power_outages.GB.northern_ireland_en.raw.{date_str}.json"
    raw_path = f"northern_ireland_en/raw/{year}/{month}/{raw_filename}"
    ...
    # Save processed JSON
    processed_filename = (
        f"power_outages.GB.northern_ireland_en.processed.{date_str}.json"
    )
    ...
    processed_path = (
        f"northern_ireland_en/processed/{year}/{month}/{processed_filename}"
    )
