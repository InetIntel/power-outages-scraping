...
class Process_Npp:
        ...
        for _, row in df.iterrows():
            try:
                ...
                data = {
                    ...
                    "event_category": str(row[10]).strip()
                    if not pd.isna(row[10])
                    else "unknown",
                    "area_affected": {
                        str(row[0]).strip() if not pd.isna(row[0]) else "unknown": str(
                            row[2]
                        ).strip()
    ...
    def save(self, data):
        filename = (
            f"power_outages.IND.npp.processed.{self.date}.{self.report_name}.json"
        )
