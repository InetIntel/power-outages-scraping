...
class Processor:
    def __init__(self, key):
        ...
        self.processed_filename = (
            f"power_outages.BD.{self.provider}.processed.{self.today}.json"
        )

        self.raw_folder_path = os.path.join(
            "/data", "bangladesh", "nesco", "raw", self.year, self.month
        )
        self.processed_folder_path = os.path.join(
            "/data", "bangladesh", "nesco", "processed", self.year, self.month
        )
    ...
    def process(self):
        ...
        with open(
            os.path.join(self.processed_folder_path, self.processed_filename), "w"
        ) as f:
